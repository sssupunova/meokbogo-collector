...
분야(프로파일)를 고르고 수집 방식을 **① 브랜드 목록 자동 · ② 카테고리 선택(네이버쇼핑
전체 카테고리) · ③ 직접 키워드** 중에서 선택해 실행하면, 결과 요약과 **상세·시드·복합**
파일 다운로드 링크가 나온다(내부적으로 `run.py` 호출). `.env` 의 네이버 API 키가 필요하다.

- 검색어당 최대 개수는 슬라이더, 눈덩이 확장은 게이지(안 함/1/2/3회)로 직관적으로 조절.
- 카테고리·직접 키워드로 넓게 모을 땐 `general`(범용) 프로파일을 쓴다.
...
- 빌트인 프로파일:
  ...
  - `general` — 범용(전체 카테고리/직접 키워드용, 브랜드 자동 미지원)
