...
2. **브랜드·제품명 시드** `products_*_seed.xlsx` — 제품 단위로 추린 먹보고 시드
...
- **제품명**: 원본 상품명에서 용량·입수·괄호·마케팅 문구를 걷어낸 읽기용 이름(브랜드 포함).
- **묶음**: `신라면 + 너구리 + 짜파게티` 같은 번들/모음(단일 제품 아님) 표시.
...
```
같은 (브랜드, 제품명)의 변형을 한 행으로 모은 것. 번들/모음은 제외한다. 먹보고 DB 시드로 바로 쓴다.
